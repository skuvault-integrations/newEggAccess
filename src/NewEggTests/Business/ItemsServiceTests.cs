...
namespace NewEggTests.Business
{
	...
	public class ItemsServiceTests : BaseBusinessTest
	{
		...
		{
			var response = await _itemsService.GetBatchInventoryAsync(
				new List<string> { TestSku1, TestSku2 },
				WarehouseLocationCountryCode,
				Mark.CreateNew(),
				CancellationToken.None);
			response.Count.Should().Be(2);
			response[0].SellerPartNumber.ToLower().Should().Be(TestSku2.ToLower());
			response[1].SellerPartNumber.ToLower().Should().Be(TestSku1.ToLower());
		}
		...
		public async Task GetBatchInventoryThatDoesntExist()
		{
			...
			var response = await this._itemsService.GetBatchInventoryAsync(
				new List<string> { sku, sku2 },
				WarehouseLocationCountryCode,
				Mark.CreateNew(),
				CancellationToken.None);
